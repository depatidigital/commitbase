...
export type TeardownStep = {
  id: TeardownStepId;
  /** what will run or be removed, shown verbatim next to the checkbox */
  detail: string;
 ...
 */
export function folderRisk(dir: string | null | undefined, others: string[]): string | null {
  if (!dir) return 'No folder was detected for this app';
  if (!dir.startsWith('/') || dir.split('/').includes('..')) return `${dir} is not a plain absolute path`;
...
export async function teardownPlan(app: Application): Promise<TeardownStep[]> {
  ...
  const noServer = server ? undefined : 'Not linked to a server — sync the apps again first';
  // the panel's own site shows up in scans; removing it would take this panel down
  const isPanel = PANEL_HOST !== '' && app.domain.toLowerCase() === PANEL_HOST ? 'This is the panel itself' : undefined;
  const placeholder = app.domain.endsWith('.pm2.local');
  const steps: TeardownStep[] = [];

  if (app.runtime === 'PM2' && app.processName) {
    steps.push({ id: 'process', detail: `pm2 delete ${app.processName} && pm2 save`, blocked: isPanel ?? noServer });
  } else if (app.runtime === 'CADDY_PROXY') {
    steps.push({
      id: 'process',
      detail: app.port ? `whatever listens on port ${app.port}` : 'the process behind the proxy',
      blocked: 'Not started by pm2, so we do not know how to stop it — stop it on the server yourself',
    });
  }

  if (!placeholder) {
    steps.push({ id: 'route', detail: `Caddy route for ${app.domain}`, blocked: isPanel ?? noServer });
    if (app.domainId) {
      steps.push({ id: 'dns', detail: `DNS record ${app.domain} → this server (Cloudflare, only if it points here)`, blocked: isPanel });
    }
  }
  ...
  if (app.runtime !== 'CADDY_PROXY') {
    ...
    steps.push({
      id: 'files',
      detail: app.rootPath ? `rm -rf ${app.rootPath}` : 'app folder',
...
export async function teardownApp(app: Application, requested: string[]): Promise<TeardownResult> {
  ...
  for (const id of requested) {
    ...
    if (step.blocked) throw new Error(`Cannot remove ${step.detail}: ${step.blocked}`);
  }
