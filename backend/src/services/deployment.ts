...
import { appBuild, BUILD_MEMORY_MAX, ensureOrgOnNode, sourceTreeUnit } from './orgProvisionService';
...
/** BUILD_MEMORY_MAX (2G, 512M) as a V8 heap size in MB, for a build outside a cgroup. */
export function buildHeapMb(limit: string = BUILD_MEMORY_MAX): number {
  const m = /^(\d+)([KMGT]?)$/.exec(limit);
  if (!m) return 2048;
  const mb = Number(m[1]) * ({ K: 1 / 1024, M: 1, G: 1024, T: 1024 * 1024, '': 1 / (1024 * 1024) }[m[2]!] ?? 1);
  return Math.max(256, Math.round(mb));
}
...
export class DeploymentService {
  ...
  async cancelDeploy(applicationId: string): Promise<boolean> {
    ...
    cancelling.add(key);
    // a static build on the panel has no unit and stops at the next step instead;
    // one on a node is a cb-build unit like any other (stop of a missing unit is harmless)
  ...
  private async buildStaticRelease(
    ...
    const sourcesDir = afs.node ? afs.sourcesDir : path.resolve(afs.sourcesDir);
  ...
  private async buildAndUploadStatic(
    ...
    if (steps.length === 0) {
      ...
      const slug = application.organization?.slug;
      if (!slug) throw new Error('This site has no organization node to build on');
      ...
      try {
        await appBuild(slug, application.sourceId ?? application.id, application.id, onOutput);
      } finally {
        await appending;
      ...
      // On the panel (a site whose org has no node yet), as the backend user.
      // No cgroup here: nice keeps it off the panel's CPU and the V8 heap cap
      // (BUILD_MEMORY_MAX, same knob as the node builds) makes a runaway build
      // die alone instead of taking the panel with it.
      ...
      const script = [...allow, install, build && `cd "$1" && ${build}`].filter(Boolean).join(' && ');
      await streamToLog('nice', ['-n', '10', 'sh', '-c', script, 'sh', workDir], buildLogPath, 600000, {
        cwd: installDir,
        env: {
          ...process.env,
          ...envVars,
          NODE_OPTIONS: [process.env.NODE_OPTIONS, envVars.NODE_OPTIONS, `--max-old-space-size=${buildHeapMb()}`].filter(Boolean).join(' '),
          HOME: buildHome,
  ...
  private async deployInner(config: DeploymentConfig): Promise<DeployResult> {
    ...
    try {
      ...
      // A static site without a node builds on the panel (lib/appFs) and needs no org there.
      if (application.organizationId) {
        const node =
          application.type === 'STATIC'
            ? await serverForApplication(application.id).catch(() => null)
            : await serverForApplication(application.id);
        if (node) await ensureOrgOnNode(application.organizationId, node.id, { userId: deployment.userId, trigger: 'deploy' });
      }
