...
/**
 * The prune step for an app that asked for it — none when the start command
 * runs a devDependency (tsx, ts-node, nodemon: the app would die on start), or
 * for a package manager without one. Pure.
 */
export function pruneOf(app: { pruneDevDeps?: boolean; startCommand?: string | null }, detected: { packageManager: string; startCommand: string | null }): string | null {
  if (!app.pruneDevDeps) return null;
  const start = app.startCommand || detected.startCommand || '';
  if (/\b(tsx|ts-node|ts-node-dev|nodemon|vite-node|bun run|next dev|vite dev)\b/.test(start)) return null;
  switch (detected.packageManager) {
    case 'pnpm':
      return 'pnpm prune --prod';
    case 'npm':
      return 'npm prune --omit=dev --no-audit --no-fund';
    case 'yarn':
      // classic only: berry has no prune (its workspaces-focus is another install)
      return 'yarn install --production --ignore-scripts --frozen-lockfile 2>/dev/null || true';
    default:
      return null;
  }
}
...
/**
 * The prune, undone when it took a package the build still points at. Next's
 * Turbopack links each server external into .next/node_modules (`<pkg>-<hash>`);
 * one listed under devDependencies dangles once pruned and the app dies with
 * ERR_MODULE_NOT_FOUND. Run from the app's folder. Pure.
 */
export const guardedPrune = (prune: string, installDir: string, install: string): string =>
  `${prune}; if [ -d .next/node_modules ] && find .next/node_modules -xtype l | grep -q .; then ` +
  `echo 'Pruning removed packages the server imports (move them to dependencies); reinstalling:'; ` +
  `find .next/node_modules -xtype l -printf '  %P\\n'; (cd ${q(installDir)} && ${install}); fi`;
      ...
      for (const app of group) {
        ...
        if (detected.type === 'PHP') {
          ...
          if (firstInstall(installCommand)) {
            ...
            // a pruned release has no devDependencies to build with — installed afresh instead
            const reusable = installDir === workDir && !pruneOf(app, detected);
        ...
        // the release keeps only what runs: devDependencies (typescript, prisma CLI,
        // @types) went into the build above and are dead weight after it
        const prune = pruneOf(app, detected);
        if (prune) {
          const install = detected.packageManager === 'npm'
            ? `( umask 000; : >> ${NPM_LOCK} ) 2>/dev/null || true; flock -w 1800 ${NPM_LOCK} sh -c ${q(installCommand)}`
            : installCommand;
          steps.push(guardedPrune(prune, installDir, install));
        }
        else if (app.pruneDevDeps) steps.push(`echo 'devDependencies kept: the start command runs one'`);
