...
router.post('/:id/import', authenticateToken, async (req: AuthenticatedRequest, res: Response) => {
  ...
  try {
    ...
    started = true;
    // releases the database and deletes the file when done. reset=1: the person
    // ticked "empty it first" — a full dump's DROPs fail on the foreign keys otherwise
    void runImport(row.id, database.id, req.file.path, { reset: req.query.reset === '1' && tables > 0 && !database.discovered });
